...
//Ain8(PE5) is Detector 1 PhaseDetector[0]
//Ain9(PE4) is Detector 2 PhaseDetector[1]
...
void ADC_In89(unsigned long data[2]){ 
  ...
  data[1] = ADC0_SSFIFO2_R&0xFFF;  // 3A) read first result  (Detector 2)
  data[0] = ADC0_SSFIFO2_R&0xFFF;  // 3B) read second result (Detector 1)
